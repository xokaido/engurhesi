<script lang="ts">
  ...
  function albumTitleFor(albumId: string, locale: string): string {
    return data.albumTitles.find((r) => r.albumId === albumId && r.locale === locale)?.title ?? '';
  }
  function videoTitleFor(videoId: string, locale: string): string {
    return data.videoTitles.find((r) => r.videoId === videoId && r.locale === locale)?.title ?? '';
  }
</script>
...
{#if form?.videoTitleSaved}<p class="ok-msg" role="status">სათაური შენახულია</p>{/if}
...
<p class="muted auto-hint">
  🌐 საკმარისია ქართული სათაური — EN/РУ ვერსიები შექმნისას ავტომატურად ითარგმნება.
</p>
...
{#each data.albums as album (album.id)}
  <div class="panel">
    ...
    <details style="margin-top: var(--sp-2);">
      ...
      <form
        method="POST"
        action="?/setAlbumTitles"
        class="actions-row"
        style="margin-top: var(--sp-2); align-items: flex-start;"
      >
        <input type="hidden" name="csrf" value={data.csrf} />
        <input type="hidden" name="album_id" value={album.id} />
        <TriField
          name="title"
          ka={albumTitleFor(album.id, 'ka')}
          en={albumTitleFor(album.id, 'en')}
          ru={albumTitleFor(album.id, 'ru')}
          placeholder="სათაური"
        />
      ...
      <tr>
        <td style="min-width: 22rem;">
          ...
          >
            ...
            <TriField
              name="title"
              ka={videoTitleFor(video.id, 'ka')}
              en={videoTitleFor(video.id, 'en')}
              ru={videoTitleFor(video.id, 'ru')}
              placeholder="სათაური"
            />
            <button class="btn btn-sm btn-outline" type="submit">შენახვა + თარგმნა</button>
          </form>
          ...
          <div class="actions-row">
            <form method="POST" action="?/publishVideo">
              ...
              <input
                type="hidden"
                name="publish"
                value={video.status === 'published' ? '0' : '1'}
              />
              <button class="btn btn-sm btn-outline" type="submit">
