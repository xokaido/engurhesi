  ...
  const [rows, files] = await Promise.all([
    db
      .select({
        ...
        titleEn: sql<
          string | null
        >`(SELECT title FROM document_i18n WHERE document_id = ${documents.id} AND locale = 'en')`,
        titleRu: sql<
          string | null
        >`(SELECT title FROM document_i18n WHERE document_id = ${documents.id} AND locale = 'ru')`
      })
      .from(documents)
...
export const actions: Actions = {
  ...
  addFile: async (event) => {
    ...
    await db
      ...
      .where(
        sql`${documentFiles.documentId} = ${documentId} AND ${documentFiles.locale} = ${locale}`
      );
    await db.insert(documentFiles).values({
      id: crypto.randomUUID(),
      documentId,
  ...
  setTitles: async (event) => {
    ...
    for (const locale of ['ka', 'en', 'ru'] as const) {
      ...
      if (existing) {
        ...
        await db.insert(documentI18n).values({ documentId, locale, title, reviewStatus });
      }
    }
    ...
    return { titleSaved: true, warning };
  },
