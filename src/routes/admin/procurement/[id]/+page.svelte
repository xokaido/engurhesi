...
{#if item.amendsId}
	<p class="notice notice-amber">
		ეს განცხადება არის ცვლილება — <a href="/admin/procurement/{item.amendsId}">თავდაპირველი განცხადება</a>
	</p>
{/if}
...
<LocaleTabs {status}>
	{#snippet panel(locale)}
		...
		<form method="POST" action="?/save" class="panel">
			...
			<div class="field">
				<span>ტექსტი</span>
				<RichTextEditor name="body" initialJson={row?.bodyJson ?? ''} />
			</div>
...
<div class="panel">
	...
	{#if data.validTransitions.length > 0 && item.status !== 'draft'}
		<form method="POST" action="?/transition" class="actions-row" style="margin-top: var(--sp-2);">
			...
			<input type="text" name="reason" placeholder="მიზეზი (სავალდებულო)" required style="flex: 1; min-width: 14rem;" />
	...
	{#if ['published', 'amended'].includes(item.status)}
		<form method="POST" action="?/amend" class="actions-row" style="margin-top: var(--sp-2);">
			<input type="hidden" name="csrf" value={data.csrf} />
			<input type="text" name="summary" placeholder="ცვლილების მოკლე აღწერა" required style="flex: 1; min-width: 14rem;" />
		...
		<ul>
			{#each data.amendments as amendment (amendment.id)}
				<li>
					...
					<span class="badge badge-neutral">{STATUS_LABEL[amendment.status] ?? amendment.status}</span>
				</li>
			{/each}
		</ul>
	</div>
