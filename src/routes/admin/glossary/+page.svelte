...
<div class="panel">
	...
	<form method="POST" action="?/create" class="actions-row" style="flex-wrap: wrap;">
		...
		<input type="text" name="term_en" placeholder="English" required />
		<input type="text" name="term_ru" placeholder="Русский" required />
...
{#each data.items as item (item.id)}
	<div class="panel">
		<form method="POST" action="?/update" class="actions-row" style="flex-wrap: wrap;">
			...
			<input type="text" name="term_en" value={item.termEn} required />
			<input type="text" name="term_ru" value={item.termRu} required />
