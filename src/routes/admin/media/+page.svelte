...
<div class="admin-head">
  ...
  <nav class="actions-row">
    ...
    <a
      class="btn btn-sm {data.kind === 'image' ? '' : 'btn-outline'}"
      href="/admin/media?kind=image">სურათები</a
    >
    <a
      class="btn btn-sm {data.kind === 'document' ? '' : 'btn-outline'}"
      href="/admin/media?kind=document">დოკუმენტები</a
    >
    <a
      class="btn btn-sm {data.kind === 'video' ? '' : 'btn-outline'}"
      href="/admin/media?kind=video">ვიდეო</a
    >
  </nav>
</div>
...
<div class="panel">
  ...
  >
    ...
    <input
      type="text"
      name="alt"
      placeholder="ალტ-ტექსტი (სურათებისთვის)"
    ...
    <div class="panel media-cell">
      ...
      {#if item.kind === 'image'}
        <form
          method="POST"
          action="?/setAlt"
          style="margin-top: var(--sp-1); display: grid; gap: 0.375rem;"
        >
          <input type="hidden" name="csrf" value={data.csrf} />
