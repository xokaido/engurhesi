<script lang="ts">
  ...
  function insertImage(mediaId: string) {
    editor
      ?.chain()
      .focus()
      .insertContent({ type: 'image', attrs: { mediaId, alt: '' } })
      .run();
    imageDialog?.close();
  }
  ...
  /** Current document JSON (kept in sync on every transaction). */
  export function getJson(): string {
    return json;
  }

  /** Replace the document programmatically (e.g. with a fresh translation). */
  export function setJson(next: string) {
    json = next;
    try {
      editor?.commands.setContent(next ? (JSON.parse(next) as Content) : '');
    } catch {
      // leave the editor as-is on malformed JSON
    }
  }
</script>
