...
export async function getArticle(db: Db, locale: Locale, slug: string) {
	...
	const i18nRows = await db
		...
		.from(articleI18n)
		.where(
			and(eq(articleI18n.articleId, row.id), inArray(articleI18n.locale, localesFor(locale)))
		);
...
function toProcurementRow(
	...
	const isOpen =
		(row.status === 'published' || row.status === 'amended') && !deadlinePassed;
	return {
		slug: row.slug,
		kind: row.kind,
		status: row.status,
...
export async function listProcurements(
	...
	const notDraft = inArray(procurements.status, [
		'published',
		'amended',
		...CLOSED_STATUSES
	]);
...
export async function getProcurement(db: Db, locale: Locale, slug: string) {
	const row = await db
		.select()
		.from(procurements)
		.where(eq(procurements.slug, slug))
		.get();
	...
	const docTitles = docIds.length
		? await db
				...
				.where(inArray(procurementDocsI18n.docId, docIds))
		: [];
...
export async function getProject(db: Db, locale: Locale, slug: string) {
	...
	const i18nRows = await db
		...
		.from(projectI18n)
		.where(
			and(eq(projectI18n.projectId, row.id), inArray(projectI18n.locale, localesFor(locale)))
		);
	const pick = pickI18n(i18nRows, locale);
	if (!pick) return null;
	const covers = await getMediaRefs(db, [row.coverMediaId ?? ''], locale);
...
export async function listPartners(db: Db, locale: Locale) {
	...
	return rows.map((row) => {
		...
		return {
			name: n.find((x) => x.locale === locale)?.name || n.find((x) => x.locale === 'ka')?.name || '',
			url: row.url,
...
export async function listOrgTree(db: Db, locale: Locale): Promise<OrgNode[]> {
	...
	const i18nRows = ids.length
		? await db
				...
				.where(
					and(
						inArray(orgUnitI18n.orgUnitId, ids),
						inArray(orgUnitI18n.locale, localesFor(locale))
					)
				)
		: [];
...
export function settingForLocale(
